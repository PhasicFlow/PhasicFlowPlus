...
void pFlow::coupling::GaussianIntegral::checkForListsConstructed()
{
    if( !listsConstructed_ )
    {
        REPORT(0)<<"Constructing neighbor lists for GaussianIntegral distribution..."<<END_REPORT;
        constructLists(1.0, maxLayers_);
        listsConstructed_ = true;
    }
}

void pFlow::coupling::GaussianIntegral::updateWeights(const Plus::procCMField<real> & parDiameter)
{
