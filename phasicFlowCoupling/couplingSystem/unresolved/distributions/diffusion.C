...
:
    self(dict, cMesh, centerMass),
    nSteps_
    (
        Foam::max(1, lookupDict<Foam::label>(dict, "nSteps"))
    ),
    standardDeviation_
    (
        lookupDict<Foam::scalar>(dict, "standardDeviation")
    ),
    intTime_(1.0),
    dt_("dt", Foam::dimTime, intTime_/nSteps_),
    DT_("DT", Foam::dimDynamicViscosity/Foam::dimDensity, standardDeviation_*standardDeviation_/4),
    smoothSolDict_("smoothSolDict")
{
    ...
    int log = dict.lookupOrDefault<int>("log", 0);
