...
)
{
    ...
    const auto& bndris = mesh().boundary();
...
void pFlow::coupling::Gaussian::checkForListsConstructed()
{
    if( !listsConstructed_ )
    {
        REPORT(0)<<"Constructing neighbor lists for Gaussian distribution..."<<END_REPORT;
...
)
{
    ...
    const Foam::vectorField& allCellCntr = mesh().cellCentres();
    ...
    for(size_t i=0; i<numPar; i++)
    {
        ...
        else
        {
            ...
            const auto& bndry = mesh().boundary()[bndryIndex];
